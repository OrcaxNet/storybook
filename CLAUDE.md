...
`migration.py` opens v1 sources read-only, keeps a consistent v1 backup for at least 30 days, upgrades a private copy, compares all Session/Story/edge counts and relationships, verifies `legacy_raw`, integrity/FKs and a real sqlite-vec lookup, then changes the registry pointer. Migration IDs are deterministic from Profile ID + logical source hash. Rollback creates an independent v1 copy and changes the same pointer, so the retained backup remains immutable and there is no dual-write window.
