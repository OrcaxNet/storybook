...
# ── 模型四元组：唯一来源是当前 Profile 的 model-config.json ──
...
def refresh_model_config():
    ...
    global EMBED_MODEL, EMBED_API_KEY_ENV, EMBED_API_KEY, EMBED_DIM
    global EMBED_CONFIG_SOURCE, EMBED_CONFIG_NORMALIZED, OLLAMA_HOST
    MODEL_CONFIG_PATH = PROFILE_PATHS.root / "model-config.json"
    MODEL_CONFIG = model_config_module.resolve(MODEL_CONFIG_PATH)
    generation, embedding = MODEL_CONFIG.generation, MODEL_CONFIG.embedding
    LLM_PROVIDER, LLM_BASE_URL = generation.provider, generation.base_url
    LLM_MODEL, LLM_API_KEY = generation.model, generation.secret
    EMBED_PROVIDER, EMBED_BASE_URL = embedding.provider, embedding.base_url
    EMBED_ADAPTER = "ollama" if embedding.protocol == "ollama" else "openai_compatible"
    EMBED_PRESET = "ollama" if EMBED_ADAPTER == "ollama" else "custom"
    EMBED_MODEL, EMBED_API_KEY = embedding.model, embedding.secret
    # The index's existing storage column holds an opaque file credential reference.
    # It is resolved from the model file / index snapshot, never os.environ.
    EMBED_API_KEY_ENV = embedding.credential_ref
    EMBED_DIM = MODEL_CONFIG.embedding_dimension or 1024
    EMBED_CONFIG_SOURCE, EMBED_CONFIG_NORMALIZED = MODEL_CONFIG.source, False
    OLLAMA_HOST = EMBED_BASE_URL
    return MODEL_CONFIG


refresh_model_config()
EMBED_TYPE = "api"
EMBED_VERSION = "story-v2-default-v1"
EMBED_REPRESENTATION = "default"
LLM_THINK = False
...
def embedding_text_leaves_device(base_url: str | None = None) -> bool:
    """Conservatively disclose endpoints that are not explicit loopback hosts."""

    hostname = (urlparse(base_url or EMBED_BASE_URL).hostname or "").lower()
